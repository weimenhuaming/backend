...
func Load(ctx context.Context, cfg config.KnowledgeBaseConf, embedder embeddings.Embedder) (vectorstores.Retriever, error) {
	_, chunkCount, err := readCollectionStats(ctx, cfg)
	if err != nil {
		return vectorstores.Retriever{}, err
	}
	if chunkCount == 0 {
		return vectorstores.Retriever{}, fmt.Errorf("Chroma collection 为空，请先调用 Build 构建知识库")
	}

	store, err := openChromaStore(ctx, cfg, embedder)
	if err != nil {
		return vectorstores.Retriever{}, fmt.Errorf("连接 Chroma 失败: %w", err)
	}
	return newRetriever(store, cfg), nil
}
...
// 返回值: 检索器，文档数量, 切片数量, 错误
func BuildIndex(ctx context.Context, cfg config.KnowledgeBaseConf, embedder embeddings.Embedder) (vectorstores.Retriever, int, int, error) {
